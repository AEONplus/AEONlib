...
class TestAstropyTime:
    def test_with_astropy_time(self):
        ...
        assert window.model_dump()["start"].replace(tzinfo=UTC) == datetime(
            2025, 4, 10, 0, 0, 0, 0, tzinfo=UTC
        )
        assert window.model_dump()["end"].replace(tzinfo=UTC) == datetime(
            2025, 4, 11, 0, 0, 0, 0, tzinfo=UTC
        )
    ...
    def test_from_datetime(self):
        ...
        """
        window = Window(
            start=datetime(2025, 4, 10, 0, 0, 0, 0, tzinfo=UTC),
            end=datetime(2025, 4, 11, 0, 0, 0, 0, tzinfo=UTC),
        )
        assert window.model_dump()["start"].replace(tzinfo=UTC) == datetime(
            2025, 4, 10, 0, 0, 0, 0, tzinfo=UTC
        )
    ...
    def test_from_json(self):
        ...
        assert window.start == Time(datetime(2025, 4, 10, 0, 0, 0, 0, tzinfo=UTC))
        assert isinstance(window.end, Time)
        assert window.end == Time(datetime(2025, 4, 11, 0, 0, 0, 0, tzinfo=UTC))
...
class TestTimeMJD:
    # TimeMJD uses TT, which Python's civil-time tzinfo cannot represent.
    def test_from_jd_float(self):
        obj = MjdObject(time=2460826.5)
        assert isinstance(obj.time, Time)
        assert obj.time == Time("2025-05-31", scale="tt")
    ...
    def test_from_json_float(self):
        ...
        assert target.time == Time("2025-05-31", scale="tt")

    def test_outputs_mjd(self):
        obj = MjdObject(time=Time("2025-05-31", scale="tt").to_datetime())
        dumped = obj.model_dump_json()
        assert dumped == '{"time":60826.0}'

    def test_from_datetime(self):
        obj = MjdObject(time=Time("2025-05-05", scale="tt").to_datetime())
        assert isinstance(obj.time, Time)
